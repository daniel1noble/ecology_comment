...
```{r simulated data, class.source='klippy'}
    ...
    # Parameters
        ...
        var_effect = 0.5  # Effect size (or within study) variance
                mu = 0.4  # Average, or overall, effect size
               rho = 0 #c(0.1, 0.2, 0.3, 0.4) # Note that, setting this to zero equates to Shinichi's approach
    ...
    # Now simulate effect sizes, assuming that the average effect size. Note here that, while we are not specificying a correlation between effect sizes from the same study explcitly using a covariance matrix, this is implied by fitting the random study effect. The degree of correlation between effects coming from the same study will depend on the intraclass correlation coefficient (i.e., ICC). In essence, if we assume that r = 0.5 between effect sizes, then this should be equivalent to a situation were var_paper and var_effect are both equal variance. At least in this case. The one additional aspect we are modelling is the correlation between sampling errors induced by non-independent effect sizes. This was not accounted for in Song et al. (2020)' simulations.
         # yi <- mu + rep(rnorm(no.paper, 0, sqrt(var_paper)), each = n.effects) + rnorm(n.effects*no.paper, 0, sqrt(var_effect)) + mvrnorm(n = 1, mu = rep(0,n.effects*no.paper), cov_vi)     
          
# It still feels weird to me that we are not modelling correlation at the level of the effect size as well. I understand that, by virtue of having a study-level random effect we are inducing a correlation, but it is not really what we are trying to argue because that approach still assumes that effect sizes within studies are independent, which I think will confuse readers. As such, I think including an explcit covariance matrix at the level of the effect size would demonstrate how effects can covary as a result of sampling covariance AND because of shared traits, for example, effects that are correlated because they are effects on maths and reading. 
