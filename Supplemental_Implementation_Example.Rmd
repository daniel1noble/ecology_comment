...
klippy::klippy(tooltip_message = 'Click to copy', tooltip_success = 'Done', position = 'right')
```

# Introduction
In this tutorial, we demonstrate, using some simple simulated data, how meta-analyst's can implement approaches for correcting inflated Type I error rates when using multilevel models with non-independent effect size data. For ease of presentation, we use simulated data to demonstrate the implementation of a few easy solutions. 
...
A very simple solution is to make use of robust variance estimators in the `clubSandwich` package [@Pustejovsky2020; @Tipon2015]. This works with `metafor` objects [@Wolfgang2010] quite elegantly. The benefit of such an approach is simply that we need not make any assumptions about what the correlation between effect sizes actually is (assuming we didn't know the true correlation) [@Tipon2015]. In addition, it also will acount for possible heteroscedascity. This solution can be implemented as follows using our MLMA model with fit in the above section.
...
With this simple (and well balanced) data, our RVE approches doesn't change the results, whcih is a good thing! In many cases, it may not. However, as is shown by the simulations of @Song2020 and ourselves [@Nakagawa2021], this will not be the case in all situations.
