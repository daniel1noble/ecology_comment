...
```{r simulated data, class.source='klippy'}
    ...
    # Now create the effect size level covariance matrix. Thsi would simulate a situation where, for example, you have effect sizes on reading and maths that are clearly correlated, but to varying extents across studies
      ...
      matrices <- list()
      for(i in 1:no.paper){
                    r <- sample(c(0.2, 0.4, 0.6, 0.8), size = 1, replace = TRUE)
                  tmp <- matrix(r, nrow = n.effects, ncol = n.effects)
            diag(tmp) <- 1
        matrices[[i]] <- tmp
      }

      sigma_yi <- as.matrix(Matrix::bdiag(matrices))
...
# I'm still not totally happy with how the simulation is running as we are technically not really modeling in the way that Song did. I realise that they did not model sampling covariance, this would have somehwat been built into their simulation though. Even still, the main idea here is that we have both, say, shared trait covariance and sampling covariance happening at once. Song didn't model sampling covarinace, and one possible reason we don't see much effect is because the total sampling variance might actually be low relative to total variance I'm going to try this...this is what I understand would be most likely to occure and this simulation makes most sense to me. 
      yi <- mu + rep(rnorm(no.paper, 0, sqrt(var_paper)), each = n.effects) + mvrnorm(n = 1, mu = rep(0,n.effects*no.paper), sigma_yi) + mvrnorm(n = 1, mu = rep(0,n.effects*no.paper), sigma_vi)     
