...
method.col = c(rgb(0,0,0),rgb(0.9,0.6,0),rgb(0.8,0.4,0),rgb(0,0.45,0.7),rgb(0.35,0.7,0.9), rgb(0.35,0.6,0.8), rgb(0.35,0.5,0.5), rgb(0.40,0.5,0.8), rgb(0.40,0.2,0.3), rgb(0.40,0.9,0.3), rgb(0.40,0.9,0.8))
...
#######################
	load("./sim_check_rerun/EqualStudy_noBayes.RData")
	...
	error_ES = array(dim=c(scenario, 11))
	error_ES.ci = array(dim=c(scenario, 11))

	# First, grab the relevant simulations for plotting
	relevant <- c(1:5, 6, 7, 8, 9, 12, 13)
...
for(i in c(1,3,7,9)){
		...
		plot(error_ES[size[i,1],]~c(1:method_num), xlim=c(0.5,34), ylim=c(0,0.13), pch=19, axes=F, col=method.col)
...
##################################
	...
	load("./sim_check_rerun/UnequalStudy_VaryTau.RData")
	...
	load("./sim_check_rerun/UnequalStudy_newsims.RData")

	#Calculate error rate and the confidence interval of error rate#
	   error_US = array(dim=c(scenario, 11))
	error_US.ci = array(dim=c(scenario, 11))
	...
	for(i in c(1,3,4,6)){
		...
		plot(error_US[size[i,1],]~c(1:method_num), xlim=c(0.5,48), ylim=c(0,0.13), pch=19, axes=F, col=method.col)
		...
		points(error.varytau[i,]~c(37:(36+method_num)), pch=19, col=method.col)
		arrows(x0=c(37:(36+method_num)), y0=error.varytau[i,]-error.ci.varytau[i,], y1=error.varytau[i,]+error.ci.varytau[i,], length=0, col=method.col)
		if(error.varytau[i,1]>0.1){
