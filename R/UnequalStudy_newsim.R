...
#Create array to store estimates from each method#
  parm.est = array(dim=c(scenario,iteration,13))
  parm.low = array(dim=c(scenario,iteration,13))
   parm.up = array(dim=c(scenario,iteration,13))
...
for(k in 1:scenario) {
	...
	# METHOD 12 = robumeta with HIER
		  mod10 = try(robu(formula=log.ratio~1, data=effect.size, studynum=paper, var.eff.size=var.log.ratio, method = "HIER"))
		parm.est[k,a,12] = as.numeric(try(mod10$reg_table$b.r)) 
		parm.low[k,a,12] = as.numeric(try(mod10$reg_table$CI.L))
		 parm.up[k,a,12] = as.numeric(try(mod10$reg_table$CI.U))

    # METHOD 13 = robumeta with HIER
		  mod11 = try(robu(formula=log.ratio~1, data=effect.size, studynum=paper, var.eff.size=var.log.ratio, method = "CORR", rho = 0.1))
		parm.est[k,a,13] = as.numeric(try(mod11$reg_table$b.r)) 
		parm.low[k,a,13] = as.numeric(try(mod11$reg_table$CI.L))
		 parm.up[k,a,13] = as.numeric(try(mod11$reg_table$CI.U))
...
save.image(file="./sim_check_rerun/UnequalStudy_newsims.RData")