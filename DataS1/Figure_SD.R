...
method.col = c(rgb(0,0,0),rgb(0.9,0.6,0),rgb(0.8,0.4,0),rgb(0,0.45,0.7),rgb(0.35,0.7,0.9), rgb(0.35,0.6,0.8), rgb(0.35,0.5,0.5), rgb(0.40,0.5,0.8), rgb(0.40,0.2,0.3), rgb(0.23,0.1,0.1))
...
#######################
	...
	sd = array(dim=c(scenario,10))
	for(i in 1:scenario){
		sd[i,] = apply(parm.est[i,,], 2, sd, na.rm=T)
	}

	# Above should be equivalent, possibly faster, to the code below:
	...
	for(i in c(1,3)){
		plot(sd[size[i,1],]~c(1:10), xlim=c(0.5,6*(scenario/3-1)+5.5), ylim=range(-0.06,0.4), pch=19, axes=F, col=method.col)
		box()
...
# Above should be equivalent, possibly faster, to the code below:
SD_result <- apply(parm.est, c(1,3), function(x) sd(x, na.rm = TRUE)) # take SD for cols for each row and method

# Check they are same to make sure we understand the code:
all(SD_result == sd)
...
for(i in c(1,3,7,9)){
	plot(sd[size[i,1],]~c(1:10), xlim=c(0.5,31.5), ylim=range(-0.06,0.4), pch=19, axes=F, col=method.col)
	box()
	if(i %in% c(1,4,7)){axis(2, at=c(0,0.15, 0.3), tck=-0.03)}
	for(j in 2:length(size[i,])){
		points(sd[size[i,j],]~c(((j-1)*11+1):((j-1)*11+10)), pch=19, col=method.col)
...
sd.varytau = array(dim=c(scenario,10))
for(i in 1:scenario){
